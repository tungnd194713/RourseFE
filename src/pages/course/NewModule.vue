    ...
    getVideoDuration(value) {
      this.videoDuration = value
		...
		async getCourseDetail() {
				const { data } = await CourseService.getCourse(this.$route.params.courseId);
				if (data) {
		...
		async createModule() {
          ...
          message: 'Hãy nhập mô tả khóa học'
				...
				try {
						...
						const response = await CourseService.createCourseModule(this.$route.params.courseId, formData)
