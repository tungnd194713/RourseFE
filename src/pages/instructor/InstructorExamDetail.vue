          ...
          <el-card>
            ...
            <el-row type="flex" justify="end" class="mt-2">
                <el-col :span="24">
                    ...
                    <el-button type="success" @click="$router.push({ name: 'InstructorTestPreview', params: { testId: $route.params.testId } })">Preview test</el-button>
                </el-col>
            </el-row>
          </el-card>
        </el-col>
...
export default {
  ...
  methods: {
    ...
    saveQuestion() {
      ...
      if (this.isEditing) {
        const newQuestion = this.updateQuestionById(this.currentQuestion.id);
        if (newQuestion) {
        ...
        if (newQuestion) {
            this.testForm.questions.push(this.currentQuestion);
        } else {
            this.$notify({
                title: 'Error',
                message: 'Something wrong'
